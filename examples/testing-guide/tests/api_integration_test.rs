...
async fn test_complete_user_lifecycle() {
    ...
    // GET created user
    let get_res = server.get(&format!("/users/{}", user_id)).await;
    ...
    // DELETE user
    let delete_res = server.delete(&format!("/users/{}", user_id)).await;

    delete_res.assert_status_ok();

    // VERIFY deletion (should return 404)
    let verify_res = server.get(&format!("/users/{}", user_id)).expect_failure().await;
...
async fn test_pagination_and_filtering() {
    ...
    let page1 = server.get("/users?page=1&size=5").await;
    ...
    let page2 = server.get("/users?page=2&size=5").await;
    ...
    let page_beyond = server.get("/users?page=10&size=5").await;
...
async fn test_error_handling() {
    ...
    let not_found = server.get(&format!("/users/{}", Uuid::new_v4())).expect_failure().await;
    ...
    server.get("/users/invalid-uuid").expect_failure().await.assert_status(StatusCode::BAD_REQUEST);
...
async fn test_concurrent_operations() {
    ...
    for i in 0..10 {
        let response = server.put(&format!("/users/{}", user_id)).json(&json!({"age": 25 + i})).await;

        response.assert_status_ok();
    }

    // Verify final state is consistent
    let final_res = server.get(&format!("/users/{}", user_id)).await;
...
async fn test_state_persistence_across_requests() {
    ...
    let initial_count: i32 = counter_str1.split("Counter: ").nth(1).unwrap().parse().unwrap();
    ...
    let final_count: i32 = counter_str2.split("Counter: ").nth(1).unwrap().parse().unwrap();
...
async fn test_partial_updates() {
    ...
    // Update only name
    let update1 = server.put(&format!("/users/{}", user_id)).json(&json!({"name": "New Name"})).await;
    ...
    // Update only age
    let update2 = server.put(&format!("/users/{}", user_id)).json(&json!({"age": 31})).await;
...
async fn test_bulk_operations() {
    ...
    let list_res = server.get("/users?size=100").await;
